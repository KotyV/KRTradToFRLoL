...
public class ChampSelectParsingTests
{
    ...
    [Theory]
    [InlineData("Krug : 아 홍길동 이새기", "Krug", "아 홍길동 이새기")]
    ...
    [InlineData("|055 Tcamj : Ni(Y5 시")]
    [InlineData("I IOI Tcamj : 1l(YS go ahne farm")]
    [InlineData("29 : 4 흰 곰 대 yc")]   // auteur purement numérique = fragment d'OCR
    ...
    {
        Assert.True(ChatLineParser.IsMostlyHangul("아 홍길동 이새기"));
    ...
    {
        var msg = _parser.Parse("[Team] 복 많은 사람 (Lee Sin): 도와줘");
        Assert.NotNull(msg);
